"""Shared synthetic multi-layer harness for the B1B integration tests
...
def make_natural_step_fn(stop_at_predicted_position: int | None = None):
    """`stop_at_predicted_position`: when the logits about to be returned
    would be used to select the token at exactly this absolute position,
    force `EOS_TOKEN_ID` to be the argmax -- deterministic, controlled
    generation length for test scenarios that need a clean, non-cap-hit
    natural run. `None` disables forcing (EOS never fires; used by tests
    that supply their own fixed token sequence and never call argmax)."""
...
def branch_step_fn(cache_state, token_id):
    """Dependency-injected `kvcot.discovery.branch_eval.StepFn`: scores a
    branch purely from its own (single-layer, all-heads) K/V cache tensor
    pair, deterministically seeded from `(cache content hash, token_id)` so
    a genuine no-op swap (bit-identical cache) reproduces bit-identical
    logits, and any real content change changes the seed."""
    key_cache, value_cache = cache_state
    content_fingerprint = int(
        hashlib.sha256(
            key_cache.detach().contiguous().numpy().tobytes() + value_cache.detach().contiguous().numpy().tobytes()
        ).hexdigest()[:8],
        16,
    )
