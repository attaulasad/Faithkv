...
Each variant returns `(prefill_fn, decode_one_fn)` (B1B-R2 §6), mirroring
`_synthetic_harness.make_step_fns`'s own split."""
...
def _process_position_schedule_shifted(
    state: HarnessState, pos: int, token_id: int, schedule_offset: int
) -> tuple[torch.Tensor, dict[int, LayerStepObservation]]:
...
def make_schedule_shifted_step_fns(schedule_offset: int, stop_at_predicted_position: int | None = None):
    """IDENTICAL to `_synthetic_harness.make_step_fns` except compaction
    fires on a schedule shifted by `schedule_offset` positions -- produces
    the SAME token trajectory (token generation itself is unaffected) but
    compaction events land at DIFFERENT absolute positions, so a Pass-2
    replay using this variant against a Pass-1 plan built from the
    unshifted schedule must find a compaction-position mismatch."""
    ...
    def prefill_fn(state: HarnessState, prompt_token_ids: Sequence[int]) -> PrefillStepResult:
        ...
        for token_id in prompt_token_ids:
            pos = state.pos
            logits, obs = _process_position_schedule_shifted(state, pos, token_id, schedule_offset)
...
def _process_position_query_salt(
    state: HarnessState, pos: int, token_id: int, query_salt: str
) -> tuple[torch.Tensor, dict[int, LayerStepObservation]]:
    layer_observations: dict[int, LayerStepObservation] = {}
...
def make_query_salt_step_fns(query_salt: str, stop_at_predicted_position: int | None = None):
    """IDENTICAL to `_synthetic_harness.make_step_fns` except the per-call
    query tensor is seeded with an extra salt -- compaction still fires at
    EXACTLY the same absolute positions (same schedule, same token
    ...
    def prefill_fn(state: HarnessState, prompt_token_ids: Sequence[int]) -> PrefillStepResult:
        ...
        for token_id in prompt_token_ids:
            pos = state.pos
            logits, obs = _process_position_query_salt(state, pos, token_id, query_salt)
            logits = _apply_eos_forcing(logits, pos + 1, stop_at_predicted_position)
            per_position_logits.append(logits)
            per_position_obs.append(obs)
            state.pos = pos + 1
